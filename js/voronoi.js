...
registerPaint("voronoi", class {
    constructor() {
        this.voronoi = new Voronoi, this.sites = [], this.diagram = null, this.drawSites = !1, this.drawVertices = !1
    }
    static get inputProperties() {
        return ["--voronoi-number-of-cells", "--voronoi-margin", "--voronoi-line-color", "--voronoi-line-width", "--voronoi-dot-color", "--voronoi-dot-size", "--voronoi-cell-colors", "--voronoi-seed"]
    }
    parseProps(o) {
        return ["--voronoi-number-of-cells", "--voronoi-margin", "--voronoi-line-color", "--voronoi-line-width", "--voronoi-dot-color", "--voronoi-dot-size", "--voronoi-cell-colors", "--voronoi-seed"].map(t => {
            const e = o.get(t);
            if ("undefined" == typeof CSSUnparsedValue) {
                if (!e.length || "" === e) return;
                switch (t) {
                    case "--voronoi-number-of-cells":
                        if ("auto" === e.toString().trim()) return "auto";
                    case "--voronoi-margin":
                    ...
                    case "--voronoi-cell-colors":
                        return e.toString().split(",").map(o => o.trim());
                    default:
                        return e.toString()
                }
            }
            if (!(e instanceof CSSUnparsedValue) || e.length) return e instanceof CSSUnitValue ? e.value : "--voronoi-cell-colors" === t ? e instanceof CSSUnparsedValue ? e.toString().split(",").map(o => o.trim()) : void console.warn('It seems that you have typehinted `--voronoi-cell-colors` as "<color>#" but unfortunately that\'s not working properly in Chrome until https://bugs.chromium.org/p/chromium/issues/detail?id=1017421 is fixed.\n\nPlease remove the @property declaration for --voronoi-cell-colors to fix this.\n\nFalling back to the default values for now.') : e.toString().trim()
        })
    }
    randomSites(o, t, e, i, r, n) {
        const l = (s = n, function () {
            s = 1831565813 + (s |= 0) | 0;
            var o = Math.imul(s ^ s >>> 15, 1 | s);
            return (((o = o + Math.imul(o ^ o >>> 7, 61 | o) ^ o) ^ o >>> 14) >>> 0) / 4294967296
        });
        for (var s, a = o * e, c = t * e, h = a, f = o - 2 * a, u = c, d = t - 2 * c, v = [], g = 0; g < r; g++) v.push({
            x: h + l() * f + l() / f,
            y: u + l() * d + l() / d,
            cellColor: i[g % i.length]
        });
        return v
    }
    paint(o, t, e) {
        let [i = 25, r = 0, n = "#000", l = 1, s = "transparent", a = 2, c = ["#66ccff", "#99ffcc", "#00ffcc", "#33ccff", "#99ff99", "#66ff99", "#00ffff"], h = 123456] = this.parseProps(e);
        "auto" === i && (i = Math.max(2, Math.floor(t.width / 30 + t.height / 30))), o.clearRect(-t.width, -t.height, 2 * t.width, 2 * t.height);
        const f = 2 * Math.PI,
            u = {
                xl: -l,
                xr: t.width + l,
        ...
        this.sites = this.randomSites(t.width, t.height, r / 100, c, i, h), this.voronoi.recycle(this.diagram), this.diagram = this.voronoi.compute(this.sites, u);
        let d = this.diagram.cells;
        for (let t = 0; t < d.length; t++) {
            ...
            if (e) {
                let t = e.halfedges,
                    i = t.length;
                if (i > 2) {
                    let r = t[0].getStartpoint();
                    o.beginPath(), o.moveTo(r.x, r.y);
                    for (let e = 0; e < i; e++) r = t[e].getEndpoint(), o.lineTo(r.x, r.y);
                    o.fillStyle = e.site.cellColor || "transparent", o.fill(), o.lineWidth = l, o.strokeStyle = n, o.stroke()
                }
                s && "transparent" != s && (o.beginPath(), o.fillStyle = s, o.arc(e.site.x, e.site.y, a, 0, f), o.fill())
            }
        }
    }
});
